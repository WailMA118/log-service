...
export const options = {
  scenarios: {
    // السيناريو الأول: ingestion مستمر بمعدل ثابت (RATE batch/sec).
    ingest: {
      executor: "constant-arrival-rate",
      rate: RATE,
      timeUnit: "1s",
      duration: DURATION,
      // مرفوع عمداً فوق RATE*4: لو زمن الاستجابة ارتفع تحت الضغط
      // (Postgres CPU saturation)، نحتاج VUs كافية لتغطية الطلبات
      // المعلّقة بدل ما k6 نفسه يصير عنق الزجاجة (Little's Law:
      // VUs المطلوبة ~= rate * latency، ورأينا latency يصل 10s+).
      preAllocatedVUs: Math.min(RATE * 3, 600),
      maxVUs: Math.min(RATE * 10, 1200),
      exec: "ingest",
    },
    // السيناريو الثاني: aggregate request واحد كل ثانية أثناء الـ ingestion،
    // يطابق "Support one aggregation request per second" في متطلبات المشروع.
    aggregate: {
      executor: "constant-arrival-rate",
      rate: 1,
      timeUnit: "1s",
      ...
      maxVUs: 10,
      exec: "aggregate",
      startTime: "5s", // فرصة قصيرة لتراكم بيانات قبل أول aggregate
    },
  },
  thresholds: {
    "http_req_duration{name:ingest}": ["p(95)<2000"],
    // الهدف الرسمي للمشروع: aggregate p95 < 1s
    "http_req_duration{name:aggregate}": ["p(95)<1000"],
